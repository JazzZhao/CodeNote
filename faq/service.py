...
def milvus_data_search(query_embedding, collection_name, company_id, vecToMilvus, ):
    ...
    recall_result = []
    # for batch_query_embedding in query_embedding:
    ...
    for hits in result:
        for hit in hits:
            print(f"hit: {hit}, answer field: {hit.entity.get('answer')}")
...
def milvus_data_insert(data, embeddings, batch_size, collection_name, company_id, vecToMilvus):
    ...
    for batch in embeddings:
        ...
        entities = [
            ...
            [data["question"][j][: text_max_len - 1] for j in range(i, cur_end, 1)],
            [data["answer"][j][: text_max_len - 1] for j in range(i, cur_end, 1)],
