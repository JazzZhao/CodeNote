...
def read_corpus_text(file_path):
    file = open(file_path)
    id2corpus = []
    for idx, data in enumerate(file.readlines()):
        question = data.strip()
        id2corpus.append({"question": question, "answer": question})
    return id2corpus

##读取数据
def read_simcse_text(path):
...
def gen_text_file(similar_text_pair_file):
    ...
    with open(similar_text_pair_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            text2similar_text[question] = answer
            texts_map.append({"text":question})
            texts.append(question)
            text_pair.append({"question": question, "answer": answer})
    return texts, text2similar_text, texts_map,text_pair
